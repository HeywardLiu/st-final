...
class MongoDB(Database):
    ...
    def __filter_anomaly(
        self, data: dict
    ):  # Filter out the data bad values, rules are defined in self.anomaly_values
        for key, value in list(data.items()):
            assert (
                type(value) != bson.objectid.ObjectId
            ), '[MongoDB] Insertion of two same data instances detected!\n \
If you\'re sure you are doing right, delete "_id" key in your input data.'
    ...
    def __transfer_time_type(data):
        assert not (
            data.get("time") is None
        ), "[MongoDB] Time was not provided in given data"
    ...
    def insert_earthquake_data(self, data: dict):
        assert (
            data.get("magnitude") is not None
        ), "[MongoDB] Magnitude for any factory was not provided in given data"
    ...
    def insert_electricity_data(self, data: dict):
        data = self.__transfer_time_type(data)
        assert not (
            data.get("region") is None
        ), "[MongoDB] Region was not provided in electricity data"
    ...
    def insert_reservoir_data(self, data: dict):
        ...
        ), "[MongoDB] Reservoir name was not provided in reservoir data"
        self.__filter_anomaly(data)
    ...
    def retrieve_earthquake_data_by_factory(
        ...
        assert (
            factory is not None
        ), "[MongoDB] Fatory was not specified in given request"
